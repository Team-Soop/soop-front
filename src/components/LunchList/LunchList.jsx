...
function LunchList({
  ...
  // 저장된 board 들고오기
  const BoardSaveQuery = useQuery(
    [`BoardSaveQuery${lunchId}`],
  ...
  // 좋아요,추천 들고오기
  const likeLunchQuery = useQuery(
    [`likeLunchQuery${lunchId}`],
      ...
      <li css={s.lunchLayout}>
        ...
        <div>
          ...
          {BoardSaveQuery.isLoading ? (
            ...
            <button
              onClick={() =>
                deleteLunchBoardSave.mutate({ boardId: lunchId, menuId: 2 })
              }
            >
              <IoBookmark />
            </button>
            ...
            <button
              onClick={() =>
                lunchBoardSave.mutate({ boardId: lunchId, menuId: 2 })
              }
            >
              <IoBookmarkOutline />
            </button>
        ...
        <div>카테고리: {categroies}</div>

        <div>프로필이미지: {profileImgUrl}</div>

        <div>닉네임: {nickName}</div>

        <div>가게이름: {placeName}</div>
        ...
        <div>
          {!!imgUrls ? (
            imgUrls.map((imgUrl, idx) => <img key={idx} src={imgUrl} />)
          ) : (
            <p>사진없음</p>
          )}
        </div>
