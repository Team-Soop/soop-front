...
function FeedCard({ feed }) {
    ...
    // 좋아요 들고오기
    const likeQuery = useQuery([`likeQuery${feed.feedId}`], () => feedGetLike(feed.feedId), {
                        ...
                        <li key={feed.feedId} css={s.feedlayout}>
                            ...
                            <div css={s.feedHeader}>
                                <div css={s.feedHeaderProfileImg}>
                                    <img src={!!feed.profileImgUrl ? feed.profileImgUrl : userImg} alt="" />
                                </div>
                            ...
                            <div css={s.feedcontents}>
                                ...
                                <div
                                    css={s.feedText}
                                    dangerouslySetInnerHTML={{ __html: sanitizer(feed.feedContent) }}
                                ></div>
                            </div>
                            {/* 저장하기 (수정, 삭제 X) */}
                            <div css={s.feedfavorite}>
                                {boardSaveQuery.isLoading ? (
                                    ...
                                    <button
                                        onClick={() => deleteLunchBoardSave.mutate({ boardId: feed.feedId, menuId: 1 })}
                                    >
                            ...
                            <div css={s.feedFooter}>
                                ...
                                <button css={s.feedFooterButton} onClick={() => setIsCommentOpen(!isCommentOpen)}>
                                    <FaRegCommentAlt />{" "}
                                    <span>{!commentCount?.totalCount ? 0 : commentCount?.totalCount}</span>
                                </button>
                                {/* 신고하기 */}
                                <button
                                    css={s.feedFooterButton}
                                    onClick={() => {
                                        isReportOpen(feed.feedId);
                                    }}
                                >
                                    <BsExclamationCircle />
                                </button>
                            ...
                            {/* 댓글 아이콘 눌렀을 때 */}
                            {isCommentOpen ? <FeedCardComment feedId={feed.feedId} /> : <></>}
                        </li>
                    </div>
                    ...
                    <div css={s.feedCardRoot}>
                        <li key={feed.feedId} css={s.feedlayout}>
                            ...
                            <div css={s.feedfavorite}>
                                ...
                                <button onClick={deleteMypageFeedButton}>
                                    <AiOutlineDelete />
                                </button>
                                {boardSaveQuery.isLoading ? (
                                    <></>
